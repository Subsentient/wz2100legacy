...
static POWER_QUEUE *PowerQueueCore[MAX_PLAYERS];


POWER_QUEUE *PowerQueue_Add(BASE_OBJECT *const Object)
{//Add an object to the Legacy power queue.
	POWER_QUEUE *Worker = NULL;
	
	if (!Object || Object->player > MAX_PLAYERS) return NULL;
	
	Worker = PowerQueueCore[Object->player];
	
	if (!PowerQueueCore[Object->player])
	{
		PowerQueueCore[Object->player] = Worker = malloc(sizeof(POWER_QUEUE));
		memset(PowerQueueCore[Object->player], 0, sizeof(POWER_QUEUE));
	}
	else
	{
		for (; Worker->psNext; Worker = Worker->psNext)
		{ /*Also check if we already have this item in the queue.*/
			if (Worker->psObject == Object) return Worker;
		}
		
		Worker->psNext = malloc(sizeof(POWER_QUEUE));
		memset(Worker->psNext, 0, sizeof(POWER_QUEUE));
		Worker->psNext->psPrev = Worker;
		Worker = Worker->psNext;
	}
	
	Worker->psObject = Object;
	
	return Worker;
}

BOOL PowerQueue_Del(const BASE_OBJECT *Object)
{ //deletes a job from the power queue.
	POWER_QUEUE *Worker = NULL;
	BOOL Found = false;
	uint8_t player;
	
	if (!Object || Object->player > MAX_PLAYERS) return false;
	
	player = Object->player;
	
	for (; Worker; Worker = Worker->psNext)
	{
		if (Worker->psObject == Object)
		{
			if (Worker == PowerQueueCore[player])
			{
				if (Worker->psNext)
				{
					PowerQueueCore[player] = Worker->psNext;
					PowerQueueCore[player]->psPrev = NULL;
					free(Worker);
				}
				else
				{
					free(PowerQueueCore[player]);
					PowerQueueCore[player] = NULL;
				}
			}
			else
			{
				Worker->psPrev->psNext = Worker->psNext;
				if (Worker->psNext) Worker->psNext->psPrev = Worker->psPrev;
				free(Worker);
			}
			Found = true;
		}
...
BOOL PowerQueue_Shutdown(uint8_t player)
{ //Shuts down the power queue for a given player.
	POWER_QUEUE *Worker = PowerQueueCore[player], *psNext = NULL;
	
	if (!Worker) return false;
	
	for (; Worker; Worker = psNext)
	{
		psNext = Worker->psNext;
		free(Worker);
	}
