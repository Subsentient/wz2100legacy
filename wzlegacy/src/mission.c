...
static void fillTimeDisplay(char	*psText, uint32_t time, BOOL bHours);
...
BOOL intAddMissionTimer(void)
{
    ...
    sFormInit.UserData = PACKint32_t_TRI(0,IMAGE_MISSION_CLOCK,IMAGE_MISSION_CLOCK_UP);;
...
BOOL intAddTransporterTimer(void)
{
    ...
    sFormInit.UserData = PACKint32_t_TRI(0, IMAGE_TRANSETA_DOWN, IMAGE_TRANSETA_UP);
...
void fillTimeDisplay(char *psText, uint32_t time, BOOL bHours)
{
    uint32_t		calcTime, inc = 0;
    ...
    else
    {
        ...
        if (bHours)
        {
            ...
            psText[inc++] = (uint8_t)('0'+ calcTime / 10);
            psText[inc++] = (uint8_t)('0'+ calcTime % 10);
...
static void intDisplayMissionBackDrop(WIDGET *psWidget, uint32_t xOffset, uint32_t yOffset, PIELIGHT *pColours)
{
    scoreDataToScreen();
}
...
static BOOL _intAddMissionResult(BOOL result, BOOL bPlaySuccess)
{
    ...
    sFormInit.style			= WFORM_PLAIN;
    sFormInit.x				= (int16_t)(0 + D_W);
    sFormInit.y				= (int16_t)(0 + D_H);
    sFormInit.width			= 640;
...
void setLandingZone(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2)
{
