...
Subsentient raised the VTOL height levels. */
#define	VTOL_HEIGHT_MIN				340
#define	VTOL_HEIGHT_LEVEL			400
...
/************************************************************************************/
/* VTOL model define | Edited by Subsentient to add a more realistic set of behaviour.*/
...
#define VTOL_SPIN_SPEED                 DEG(100)
...
static void moveCalcDroidSlide(DROID *psDroid, int *pmx, int *pmy)
{
	...
	for (std::vector<BASE_OBJECT *>::const_iterator gi = list.begin(); gi != list.end(); ++gi)
	{
		...
		if (psObj->type == OBJ_DROID)
		{
			...
			if (psDroid->droidType == DROID_CYBORG || psDroid->droidType == DROID_CYBORG_SUPER)
			{
				//We want cyborgs to pass through stuff and be passed through. -Subsentient
				continue;
			}
...
static Vector2i moveGetObstacleVector(DROID *psDroid, Vector2i dest)
{
	...
	for (BASE_OBJECT *psObj = gridIterate(); psObj != NULL; psObj = gridIterate())
	{
		...
		{
			//We want cyborgs to pass through stuff and be passed through. -Subsentient
			if (cyborgDroid(psDroid) != cyborgDroid(psObstacle))
			{ 
			continue;
			}
		}
