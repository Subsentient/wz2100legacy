...
//Various edits by Subsentient, including making a new default config file name.
static const char *fileName = "legacyconfig.txt";
...
bool loadConfig()
{
	...
	if (!ini.contains("masterserver_name")) {
	//Give us a message if we have no configuration file. -Subsentient
	printf("\nATTENTION: None or damaged configuration file found. Assuming this is the first start of the game.\n\n**BTW, you really might want to look at the options screen.\n"); }	if (ini.contains("voicevol")) sound_SetUIVolume(ini.value("voicevol").toDouble() / 100.0);
	...
	game.power = ini.value("power", LEV_HI).toInt();
	game.base = ini.value("base", CAMP_CLEAN).toInt();
...
bool reloadMPConfig(void)
{
	...
	game.power = ini.value("power", LEV_HI).toInt(); //Subsentient did it
	game.base = ini.value("base", CAMP_CLEAN).toInt(); //Here too
	game.alliance = ini.value("alliance", NO_ALLIANCES).toInt();
