		...
		for (i=0; i<MaxGames; i++)							// draw games
		{
			...
			if (NetPlay.games[i].desc.dwSize !=0)
			{
				...
				{ //Give us useful tooltips. -Subsentient
					ssprintf(tooltipbuffer[i], _("Game Name: \"%s\", Host: \"%s\", Map: \"%s\", IP: \"%s\""), NetPlay.games[i].name, NetPlay.games[i].hostname, NetPlay.games[i].mapname, NetPlay.games[gameNumber].desc.host);
...
static void addGameOptions()
{
	...
	addMultiBut(psWScreen,MULTIOP_FOG,MULTIOP_FOG_OFF,MCOL3,2,MULTIOP_BUTW,MULTIOP_BUTH, _("Distance Fog"),IMAGE_FOG_ON,IMAGE_FOG_ON_HI,true);
	if(bRevealActive)
	{
...
static void addAiChooser(int player)
{
	...
	sButInit.x = 7;
	sButInit.width = MULTIOP_PLAYERWIDTH + 1;
	sButInit.height = 16; //Make AI chooser smaller. Gives more room. -Subsentient
	sButInit.pDisplay = displayAi;

	const int aibuttoncounter = 21;
	int aibuttonspace = 4; //Revert the previous method of decreasing spacers for the AI boxes and use a diff variable for readability. -Subsentient
	// Open button
	if (NetPlay.bComms)
	{
...
static void disableMultiButs(void)
{
	...
	{		
			...
			//Fix scav button pretending to be clickable to someone who joined an MP game. -Subsentient
