...
static void scanDataDirs( void )
{
	...
	// version-independent location for video files
        // Subsentient added the legacy part.
	registerSearchPath("/Library/Application Support/Warzone 2100 Legacy/", 1);
#endif
	...
	if( !PHYSFS_exists("gamedesc.lev") )
	{
		...
		if( !PHYSFS_exists("gamedesc.lev") )
		{
			...
			sstrcpy(tmpstr, prefix);
			sstrcat(tmpstr, "/share/warzone2100legacy/");
...
// for backend detection | Deprecated since we are now pure QT. -Subsentient
...
int realmain(int argc, char *argv[])
{
	...
	if (!customDebugfile)
	{
		...
		// log name is logs/(or \)WZlog-MMDD_HHMMSS.txt | Subsentient changed the log file name and added the year.
	...
	// NOTE: it is now safe to use debug() calls to make sure output gets captured. | Changed to WZ Legacy by Subsentient | Also removed old backend stuff
	check_Physfs();
