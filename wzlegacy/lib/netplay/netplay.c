 ...
 ************************************************************************************
**/
char VersionString[VersionStringSize] = VERSIONNUM;   // used for display in the lobby, not the actual version check
static int NETCODE_VERSION_MAJOR = NetcodeMajor;                // major netcode version, used for compatibility check
static int NETCODE_VERSION_MINOR = NetcodeMinor;               // minor netcode version, used for compatibility check
...
static bool upnp_add_redirect(int port)
{
	...
	r = UPNP_AddPortMapping(urls.controlURL, data.servicetype,
							port_str, port_str, lanaddr, "Warzone 2100 Legacy", "TCP", 0);
...
static void NETcheckPlayers(void)
{
	...
	for (i = 0; i < MAX_PLAYERS ; i++)
	{
		...
		{
			continue;    // not allocated means that it most like it is a AI player
		}
				 ...
				 uint32_t plyrs)	// # of players.
{
	...
	if(!socket_set)
	{
		socket_set = allocSocketSet(MAX_CONNECTED_PLAYERS);
	}
	if (socket_set == NULL)
	{
		debug(LOG_ERROR, "Cannot create socket set: %s", strSockError(getSockErr()));
...
unsigned NETfindGame(void)
{
	...
	{
		LastHostTime = ntohl(LastHostTime);
	}
...
BOOL NETlobbyChatRead(char *OutBuf, uint32_t MaxSize)
{
	...
	if (!LobbyChatToken || !LobbyRecvSock || !LobbyRecvSockSet)
	{
		return false;
	}
