...
BOOL scrOrderGroupLoc(void)
{
	...
	orderGroupLoc(psGroup, order, (uint32_t)x, (uint32_t)y);
...
BOOL scrOrderDroidLoc(void)
{
	...
	orderDroidLoc(psDroid, order, (uint32_t)x, (uint32_t)y);
...
BOOL scrOrderDroidStatsLoc(void)
{
	...
	if (!IsPlayerStructureLimitReached(psDroid->player))
	{
		...
		orderDroidStatsLoc(psDroid, order, psStats, (uint32_t)x, (uint32_t)y);
	}
...
{
	int32_t				player, bias;//,timeToResearch;//,*x,*y;
	uint16_t				i;
...
BOOL scrSkDifficultyModifier(void)
{
	 ...
	 * (0 - player disabled, 20 - max value, uint8_t_MAX = autogame human)
	 */
	if (game.skDiff[player] <= 1 || game.skDiff[player] == uint8_t_MAX)
	{
		return true;
	}
